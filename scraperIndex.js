...
var houselinks = {
  ...
  rdamapartments: 'https://rotterdamapartments.com/en/Rental-apartments/filter/Rotterdam',
  ...
  perfectrent: 'https://www.perfectrent.nl/nl/huren/aanbod/rentals?view=&order=added&accommodation%5B%5D=apartment&accommodation%5B%5D=room&priceMin=0&priceMax=1000&persons=1&order=added&direction=desc&&order=added&direction=desc&',
  deblooisvastgoed: 'https://www.deblooisvastgoed.nl/huuraanbod/?filter_city=Rotterdam&filter_min_price=&filter_max_price=900&filter_interior=&filter_surface=',
  ...
  houseSelect: 'https://www.hsmakelaars.nl/woningaanbod/huur/land-nederland/gemeente-rotterdam?locationofinterest=Rotterdam&pricerange.maxprice=900',
  ...
  homerent: 'https://www.homerent.nl/nieuw-aanbod.html',
  indestad: 'https://www.indestad.nl/huurwoningen/?wpp_search%5Bpagination%5D=on&wpp_search%5Bper_page%5D=10&wpp_search%5Bstrict_search%5D=false&wpp_search%5Bproperty_type%5D=direct_aanbod&wpp_search%5Bprice%5D%5Bmin%5D=&wpp_search%5Bprice%5D%5Bmax%5D=&wpp_search%5Barea%5D%5Bmin%5D=&wpp_search%5Barea%5D%5Bmax%5D=&wpp_search%5Bplaats%5D%5B0%5D=Rotterdam',
  ...
  immobilia: 'https://www.immobilia.nl/nl/aanbod?koophuur=huur&plaats=Rotterdam&order=datum&direction=asc',
  lankhuijzen: 'https://www.lankhuijzen.nl/aanbod/woningaanbod/ROTTERDAM/-1000/huur/',
  ...
  athomevastgoed: 'https://www.athomevastgoed.nl/woningaanbod?order=published_at_desc&location=&_labels%5Blocation%5D=&category=all&furnishing=all&number_of_bedrooms=&ray=0&min_price=&max_price=900&area=',
  korrektwonen: 'http://korrektwonen.nl/properties/?filter_location=&filter_type=&filter_contract_type=54&filter_price_from=&filter_price_to=900&filter_sort_by=published&filter_order=DESC',
  rotsvast: 'https://www.rotsvast.nl/en/property-listings/?type=2&city=Rotterdam&maximumPrice[2]=1000',
  maxProperty: 'https://www.maxpropertyrental.com/woningaanbod/huur/land-nederland/gemeente-rotterdam?locationofinterest=Rotterdam&pricerange.maxprice=900'
}
...
var rdamapartments = function (){
  ...
  if (fs.existsSync(websitePathName+'index.html')) {
    ...
    oldFirstHouseLink = oldWebsiteHtml('.housecontainer .homeitem .houseimage').first().attr('href')
  ...
  return scrape(options).then((result) => {
    ...
    if (newFirstHouseLink !== oldFirstHouseLink && newFirstHouseLink) {
...
var houseSelect = function(callback){
  ...
  if (fs.existsSync(websitePathName+'index.html')) {
    ...
    oldFirstHouseLink = oldWebsiteHtml('.sys-page-of-properties .objectcontainer .object_data_container').first().attr('href')
    // remove the folder since new data should be written
    deleteFolderRecursive(websitePathName);
  }
...
var vhpn = function(callback){
  ...
  return scrape(options).then((result) => {
    ...
    if (newFirstHouseLink !== oldFirstHouseLink && newFirstHouseLink && newFirstHouseLink) {
...
var athomevastgoed = function(){
  ...
  return scrape(options).then((result) => {
    ...
    newFirstHouseLink = newWebsiteHtml('.infinite .properties__list .properties__item a').first().attr('href')
...
var maxProperty = function(){
  var websiteName = 'maxProperty'
  var websitePathName = getPathName(websiteName)
  var oldWebsiteHtml = ''
  var oldFirstHouseLink = ''
  var newWebsiteHtml = ''
  ...
  if (fs.existsSync(websitePathName+'index.html')) {
    //get the html
    oldWebsiteHtml = cheerio.load(fs.readFileSync(websitePathName+'index.html', 'utf8'));
    oldFirstHouseLink = oldWebsiteHtml('.sys-page-of-properties .objectcontainer .imagecontainer .sys-image-container a').first().attr('href')
  ...
  return scrape(options).then((result) => {
    ...
    if (newFirstHouseLink !== oldFirstHouseLink && newFirstHouseLink) {
      let text = "There is a new link in https://www.maxpropertyrental.com" + newFirstHouseLink
      console.log(text);
      if(isProduction){
        request('https://api.telegram.org/bot540567822:AAEQjj5l_kIhsfQjMIWopg-Loly0ZpIsrE0/sendMessage?chat_id=503848682&text='+ encodeURI(text), function (error, response, body) {});
        request('https://api.telegram.org/bot540567822:AAEQjj5l_kIhsfQjMIWopg-Loly0ZpIsrE0/sendMessage?chat_id=578223398&text='+ encodeURI(text), function (error, response, body) {});
